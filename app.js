...
mongoose.connect("mongodb+srv://rajkumarballepu:Rajkumar%403772@magodb.tqwe6lx.mongodb.net/blog")
...
app.post('/compose', (req, res) => {
  ...
  post.save().then(() => {
    res.render('compose.ejs', {message: 'Posted successfully'});
  }).catch(err => {
    res.render('compose.ejs', {message: 'Internal error'});
  });
});
...
app.get('/post/:title', (req, res) => {
  ...
  Post.findOne({title: req.params.title}).then((post) => {
    ...
    res.render('post.ejs', {message: "Internal error: " + err.message});
  });
});
